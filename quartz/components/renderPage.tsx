...
export function pageResources(
  ...
  // Use `no-cache` to make sure the browser revalidates the index so new content appears in navigation.
  const contentIndexScript = `const fetchData=new Promise((resolve,reject)=>{const start=()=>{const loadGz=()=>fetch("${gzPath}",{cache:"no-cache"}).then(async(res)=>{if(!res.ok)throw new Error("content index");if(!("DecompressionStream"in window))throw new Error("no gzip support");const stream=res.body??new Response(await res.arrayBuffer()).body;if(!stream)throw new Error("empty response");const decompressedStream=stream.pipeThrough(new DecompressionStream("gzip"));const text=await new Response(decompressedStream).text();return JSON.parse(text)});const loadJson=()=>fetch("${contentIndexPath}",{cache:"no-cache"}).then((res)=>{if(!res.ok)throw new Error("content index");return res.json()});loadGz().catch(()=>loadJson()).then(resolve,reject)};if(document.readyState==="loading"){window.addEventListener("DOMContentLoaded",start,{once:true})}else{start()}})`
    ...
    <html lang={lang} dir={direction}>
      ...
      <body data-slug={slug}>
        <div id="quartz-root" class="page">
          <Body {...componentData}>
            {LeftComponent}
            <main id="main-content" role="main" class="center">
